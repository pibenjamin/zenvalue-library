        ...
        return $table
            ->query(
                Loan::query()
                    ...
                    //->whereIn('loans.status', ['in_progress', 'returned'])
                    ->latest('borrowed_at')
                    ->selectRaw('borrowers.id, borrowers.name, COUNT(*) as total_books')
                    ->groupBy('borrowers.id', 'borrowers.name')
            )
            ->heading('Qui emprunte mes livres actuellement ? 👥')
            ->description('Cette liste affiche les personnes qui empruntent vos livres.')
            ->columns([
